#!/usr/bin/env bash
#
# Install Fuse from GitHub Releases.
#
...
#
# Note: Fuse embeds Grove as a library — no separate grove installation is
# required. Install grove separately only if you want the grove CLI.
set -euo pipefail
...
printf '\n%s %s installed. Open a new terminal or run:\n  export PATH="%s:$PATH"\n\nNext: fuse install  (register git merge driver globally)\n' \
